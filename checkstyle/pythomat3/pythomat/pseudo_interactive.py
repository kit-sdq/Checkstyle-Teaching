...
def analyze_error_message(document, commandline, input_lines, output_lines, expected_message):
    ...
    with document.div({'class' : "window"}):
        ...
        with document.div({'class' : "console"}):
            ...
            for pair in itertools.zip_longest([x.strip() for x in input_lines], [x.strip() for x in output_lines], ["Error, ..."], fillvalue=""):
...
def analyze_regular_output(document, commandline, input_lines, output_lines, expected_lines):
    ...
    with document.div({'class' : "window"}):
        ...
        with document.div({'class' : "console"}):
            ...
            for pair in itertools.zip_longest([x.strip() for x in input_lines], [x.strip() for x in output_lines], [x.strip() for x in expected_lines], fillvalue=""):
...
def extract_testcases(zip_filename):
    ...
    for test_file in test_files:
        ...
        test_json = json.loads(str(zip_file.read(test_file)))
