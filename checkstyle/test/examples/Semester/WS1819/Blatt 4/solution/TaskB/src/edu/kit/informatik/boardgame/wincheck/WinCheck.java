...
import edu.kit.informatik.boardgame.board.Board;
import edu.kit.informatik.boardgame.board.Player;

/**
 * Contains necessary methods for a win check.
 *
 * @author Thomas Weber
 * @version 1.0
 */
public interface WinCheck {
    ...
    /**
     * Alters the column coordinate in a direction dependent on the win check
     * type.
     *
    ...
    /**
     * Checks if the player has won with his recent move in a way that the
    ...
    default boolean check(Board board, Player player, int i, int j) {
        ...
        for (Direction direction : Direction.values()) {
            for (int offset = 1; offset < ROW_LENGTH; offset++) {
                ...
                //getOccupyingPlayer returns null if the coordinates are out
                // of bounds or the cell is not occupied
                //-> smth.equals(null) is always false
                if (player
                        .equals(board.getOccupyingPlayer(alteredI, alteredJ))) {
                    counter++;
                } else {
