...
import edu.kit.informatik.CellOutOfBoundsException;
    ...
    default boolean check(Board board, Player player, int i, int j) {
        ...
        for (Direction direction : Direction.values()) {
            for (int offset = 1; offset < ROW_LENGTH; offset++) {
                ...
                try {
                    occupyingPlayer = board.getOccupyingPlayer(alteredI, alteredJ);
                } catch (CellOutOfBoundsException e) {
                    // We can stop checking because we reached the border
                    break;
                }
                
                if (player.equals(occupyingPlayer)) {
                    counter++;
                } else {
