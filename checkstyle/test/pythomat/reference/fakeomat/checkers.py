...
class ZipDirectoryChecker(Checker):
    ...
    def run(self):
        ...
        for content in filter(lambda x: not os.path.splitext(x)[1] == ".swp", os.listdir(self.arguments)):
...
class CompilerChecker(Checker):
    ...
    def run(self):
        ...
        for directory, s, filenames in os.walk(self.tempdir):
            for filename in filter(lambda x: os.path.splitext(x)[1] == ".java", filenames):
        ...
        jar_files = []
        for directory, s, filenames in os.walk(self.tempdir):
            for filename in filter(lambda x: os.path.splitext(x)[1] == ".jar", filenames):
