...
def checkstyle(checker, filenames, config):
    ...
    filenames = [filename[1:-1] if filename.startswith("'") and filename.endswith("'") else filename for filename in
                 filenames]
    ...
    process = subprocess.Popen(command_line, env=env, shell=False, stdin=subprocess.PIPE, stdout=subprocess.PIPE,
...
def print_grouped_messages(checker, group, messages):
    ...
    with test.context():
        ...
        split_messages = [(os.path.relpath(filename).strip() + ':' + line + ':' + column, info.strip()) for
                          filename, line, column, info in split_messages]

        # TODO: don't display more than approximately 10-20 messages per
        # category to limit output length.
        output.print_messages(checker.document,
                              (output.Message(severity, info, location) for location, info in split_messages),
                              title="Reported style issues")
